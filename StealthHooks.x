// StealthHooks.x - MediaPlaybackUtils v1.4.6
// Скрывает твик из списка dylib — только для приложений из plist
...
static BOOL      _filter_built   = NO;

// ИСПРАВЛЕНИЕ: dispatch_once убирает race condition
...
static uint32_t hook_dyld_image_count(void) {
    ...
    if (!_filter_built) _stealth_rebuild_filter();
...
%hook NSString
+ (instancetype)stringWithContentsOfFile:(NSString *)path encoding:(NSStringEncoding)enc error:(NSError **)err {
    if (!_stealth_is_trusted() && path) {
        if (![path containsString:@"dpkg/status"] && ![path containsString:@"dpkg/info"]) {
            if ([path containsString:@"MobileSubstrate"] || [path hasPrefix:@"/var/jb/usr/lib/TweakInject"]) {
                if (err) *err = nil;
                return @"";
            }
        }
...
+ (instancetype)stringWithContentsOfFile:(NSString *)path usedEncoding:(NSStringEncoding *)enc error:(NSError **)err {
    if (!_stealth_is_trusted() && path) {
        if (![path containsString:@"dpkg/status"] && ![path containsString:@"dpkg/info"]) {
            if ([path containsString:@"MobileSubstrate"] || [path hasPrefix:@"/var/jb/usr/lib/TweakInject"]) {
                if (err) *err = nil;
...
+ (NSArray<NSBundle *> *)allBundles {
    ...
    for (NSBundle *b in orig) {
        ...
        if (bid && ([bid containsString:@"proximacore"] || [bid containsString:@"mediaplaybackutils"])) continue;
...
%ctor {
    @autoreleasepool {
        ...
        // Это предотвращает краши в Telegram, банковских и других приложениях
